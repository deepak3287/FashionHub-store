...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ error: error.message || "Unable to update coupon" }, { status: 400 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ error: error.message || "Unable to delete coupon" }, { status: 400 });
  }
}
